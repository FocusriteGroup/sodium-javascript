...
function crypto_kx_keypair(pk, sk) {
  assert(
    pk.byteLength === crypto_kx_PUBLICKEYBYTES,
    "pk must be 'crypto_kx_PUBLICKEYBYTES' bytes"
  );
  assert(
    sk.byteLength === crypto_kx_SECRETKEYBYTES,
...
function crypto_kx_seed_keypair(pk, sk, seed) {
  ...
  assert(
    seed.byteLength === crypto_kx_SEEDBYTES,
...
function crypto_kx_client_session_keys(
  ...
  assert(
    client_pk.byteLength === crypto_kx_PUBLICKEYBYTES,
    "client_pk must be 'crypto_kx_PUBLICKEYBYTES' bytes"
  );
  assert(
    client_sk.byteLength === crypto_kx_SECRETKEYBYTES,
    "client_sk must be 'crypto_kx_SECRETKEYBYTES' bytes"
  );
  assert(
    server_pk.byteLength === crypto_kx_PUBLICKEYBYTES,
    "server_pk must be 'crypto_kx_PUBLICKEYBYTES' bytes"
  );
  ...
  const hashInput = new Uint8Array(
    q.length + client_pk.length + server_pk.length
  );
  hashInput.set(q, 0);
  ...
  tx.set(
    keys.subarray(crypto_kx_SESSIONKEYBYTES, 2 * crypto_kx_SESSIONKEYBYTES)
  );
